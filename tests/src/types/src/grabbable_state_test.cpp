...
TEST_CASE("grabbable_state json") {
  ...
  for (uint8_t i = static_cast<uint8_t>(krbn::grabbable_state::ungrabbable_temporarily_reason::none);
       i < static_cast<uint8_t>(krbn::grabbable_state::ungrabbable_temporarily_reason::end_);
       ++i) {
    auto t = krbn::grabbable_state::ungrabbable_temporarily_reason(i);

    nlohmann::json json = t;
    REQUIRE(json.get<krbn::grabbable_state::ungrabbable_temporarily_reason>() == t);
  }
