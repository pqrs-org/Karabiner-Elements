...
suspended_(false)  {
  ...
  device_grabber::~device_grabber(void) {
  ...
  void device_grabber::start_grabbing(const std::string& user_core_configuration_file_path) {
    gcd_utility::dispatch_sync_in_main_queue(^{
      mode_ = mode::grabbing;
  ...
  void device_grabber::stop_grabbing(void) {
    gcd_utility::dispatch_sync_in_main_queue(^{
  ...
  void device_grabber::grab_devices(void) {
    gcd_utility::dispatch_sync_in_main_queue(^{
  ...
  void device_grabber::ungrab_devices(void) {
    gcd_utility::dispatch_sync_in_main_queue(^{
      for (auto&& it : hids_) {
        (it.second)->ungrab();
      }
  ...
  void device_grabber::suspend(void) {
    gcd_utility::dispatch_sync_in_main_queue(^{
      if (!suspended_) {
  ...
  void device_grabber::resume(void) {
    gcd_utility::dispatch_sync_in_main_queue(^{
      if (suspended_) {
  ...
  void device_grabber::virtual_hid_device_client_disconnected_callback(void) {
    stop_grabbing();
  }

  void device_grabber::static_device_matching_callback(void* _Nullable context, IOReturn result, void* _Nullable sender, IOHIDDeviceRef _Nonnull device) {
  ...
  void device_grabber::device_removal_callback(IOHIDDeviceRef _Nonnull device) {
    if (!device) {
      return;
    }
  ...
  void device_grabber::value_callback(human_interface_device& device,
  ...
  human_interface_device::grabbable_state device_grabber::is_grabbable_callback(human_interface_device& device) {
  ...
  void device_grabber::grabbed_callback(human_interface_device& device) {
    // set keyboard led
    if (event_tap_manager_) {
      bool state = false;
  ...
  void device_grabber::ungrabbed_callback(human_interface_device& device) {
    // stop key repeat
    event_manipulator_.stop_key_repeat();
  }

  void device_grabber::disabled_callback(human_interface_device& device) {
    // stop key repeat
    event_manipulator_.stop_key_repeat();
  }

  void device_grabber::caps_lock_state_changed_callback(bool caps_lock_state) {
    event_manipulator_.set_caps_lock_state(caps_lock_state);
    update_caps_lock_led(caps_lock_state);
  }

  void device_grabber::update_caps_lock_led(bool caps_lock_state) {
    // Update LED.
    for (const auto& it : hids_) {
  ...
  bool device_grabber::is_keyboard_connected(void) {
    for (const auto& it : hids_) {
  ...
  bool device_grabber::is_pointing_device_connected(void) {
    for (const auto& it : hids_) {
  ...
  boost::optional<const core_configuration::profile::device&> device_grabber::find_device_configuration(const human_interface_device& device) {
  ...
  bool device_grabber::get_disable_built_in_keyboard_if_exists(const human_interface_device& device) {
    if (auto s = find_device_configuration(device)) {
  ...
  bool device_grabber::need_to_disable_built_in_keyboard(void) {
    for (const auto& it : hids_) {
  ...
  void device_grabber::enable_devices(void) {
    for (const auto& it : hids_) {
  ...
  void device_grabber::output_devices_json(void) {
