...
public:
  template <typename E>
  static std::ostream& output_enum(std::ostream& stream, const std::string& name, const E& value) {
    stream << name << " (" << static_cast<typename std::underlying_type<E>::type>(value) << ")";
    return stream;
  }
