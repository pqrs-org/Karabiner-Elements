...
public:
  ...
  static nlohmann::json to_json(const pqrs::karabiner_virtual_hid_device::hid_report::keys& v,
    ...
    for (const auto& k : v.get_raw_value()) {
      ...
      if (auto key_code = make_key_code(usage_page, pqrs::osx::iokit_hid_usage::value_t(k))) {
        json.push_back(make_key_code_name(*key_code));
        continue;
      }

      if (auto consumer_key_code = make_consumer_key_code(usage_page, pqrs::osx::iokit_hid_usage::value_t(k))) {
