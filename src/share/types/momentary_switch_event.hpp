...
public:
  ...
  [[nodiscard]] std::optional<krbn::modifier_flag> make_modifier_flag() const {
  ...
  [[nodiscard]] bool modifier_flag() const {
  ...
  [[nodiscard]] bool pointing_button() const {
  ...
  // - false: D-pad
  [[nodiscard]] bool interrupts_key_repeat() const {
    if (!valid() ||
        modifier_flag() ||
        caps_lock() ||
        pointing_button() ||
