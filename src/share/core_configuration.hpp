...
public:
  ...
  public:
    ...
    public:
      ...
      const std::vector<std::pair<std::string, std::vector<std::string>>>& get_pairs(void) const {
        return pairs_;
      }

      void replace_pair(size_t index, const std::string& from, const std::vector<std::string>& to) {
        if (index < pairs_.size()) {
          pairs_[index].first = from;
          pairs_[index].second = to;
        }
      ...
      void replace_second(const std::string& from, const std::vector<std::string>& to) {
        for (auto&& it : pairs_) {
          if (it.first == from) {
            it.second = to;
            return;
    ...
    profile(const nlohmann::json& json) : json_(json),
                                          ...
                                          standalone_keys_(json.find("standalone_keys") != json.end() ? json["standalone_keys"] : nlohmann::json()),
                                          one_to_many_mappings_(json.find("one_to_many_mappings") != json.end() ? json["one_to_many_mappings"] : nlohmann::json()),
      ...
      }
      {
        ...
        if (json.find(key) != json.end() && json[key].is_object()) {
          for (auto it = json[key].begin(); it != json[key].end(); ++it) {
            if (it.value().is_string()) {
              standalone_keys_.replace_second(it.key(), it.value());
            }
          }
        }
      }
      {
        ...
        if (json.find(key) != json.end() && json[key].is_object()) {
          for (auto it = json[key].begin(); it != json[key].end(); ++it) {
            if (it.value().is_array()) {
              one_to_many_mappings_.replace_second(it.key(), it.value());
            }
          }
        }
      }
