...
void libkrbn_load_custom_environment_variables() {
...
void libkrbn_enqueue_callback(void (*callback)()) {
...
void libkrbn_services_register_core_daemons() {
  krbn::services_utility::register_core_daemons();
}

void libkrbn_services_register_core_agents() {
  krbn::services_utility::register_core_agents();
}

void libkrbn_services_bootout_old_agents() {
  krbn::services_utility::bootout_old_agents();
}

void libkrbn_services_restart_console_user_server_agent() {
  krbn::services_utility::restart_console_user_server_agent();
}

void libkrbn_services_unregister_all_agents() {
  krbn::services_utility::unregister_all_agents();
}

bool libkrbn_services_daemons_enabled() {
  return krbn::services_utility::core_daemons_enabled() == true;
}

bool libkrbn_services_agents_enabled() {
...
void libkrbn_updater_check_for_updates_stable_only() {
  krbn::update_utility::check_for_updates_stable_only();
}

void libkrbn_updater_check_for_updates_with_beta_version() {
  krbn::update_utility::check_for_updates_with_beta_version();
}

void libkrbn_launch_event_viewer() {
  krbn::application_launcher::launch_event_viewer();
}

void libkrbn_launch_settings() {
  krbn::application_launcher::launch_settings();
}

void libkrbn_killall_settings() {
...
void libkrbn_enable_configuration_monitor() {
  if (auto manager = libkrbn_components_manager_) {
    manager->enable_configuration_monitor();
  }
}

void libkrbn_disable_configuration_monitor() {
  if (auto manager = libkrbn_components_manager_) {
    manager->disable_configuration_monitor();
  }
}

void libkrbn_register_core_configuration_updated_callback(libkrbn_core_configuration_updated_t callback) {
  if (auto manager = libkrbn_components_manager_) {
...
void libkrbn_unregister_core_configuration_updated_callback(libkrbn_core_configuration_updated_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto m = manager->get_libkrbn_configuration_monitor()) {
...
void libkrbn_enable_complex_modifications_assets_manager() {
  if (auto manager = libkrbn_components_manager_) {
...
size_t libkrbn_complex_modifications_assets_manager_get_files_size() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto m = manager->get_complex_modifications_assets_manager()) {
...
void libkrbn_enable_file_monitors() {
  if (auto manager = libkrbn_components_manager_) {
    manager->enable_file_monitors();
  }
}

void libkrbn_disable_file_monitors() {
  if (auto manager = libkrbn_components_manager_) {
    manager->disable_file_monitors();
  }
}

void libkrbn_register_file_updated_callback(const char* file_path,
...
void libkrbn_unregister_file_updated_callback(const char* file_path,
                                              libkrbn_file_updated_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto m = manager->get_libkrbn_file_monitors()) {
...
void libkrbn_enable_log_monitor() {
  if (auto manager = libkrbn_components_manager_) {
    manager->enable_log_monitor();
  }
}

void libkrbn_disable_log_monitor() {
  if (auto manager = libkrbn_components_manager_) {
    manager->disable_log_monitor();
  }
}

void libkrbn_register_log_messages_updated_callback(libkrbn_log_messages_updated_t callback) {
  if (auto manager = libkrbn_components_manager_) {
...
void libkrbn_unregister_log_messages_updated_callback(libkrbn_log_messages_updated_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto m = manager->get_libkrbn_log_monitor()) {
...
void libkrbn_enable_core_service_daemon_client() {
  if (auto manager = libkrbn_components_manager_) {
    manager->enable_core_service_daemon_client();
  }
}

void libkrbn_disable_core_service_daemon_client() {
  if (auto manager = libkrbn_components_manager_) {
    manager->disable_core_service_daemon_client();
  }
}

void libkrbn_core_service_daemon_client_async_start() {
  if (auto manager = libkrbn_components_manager_) {
...
void libkrbn_register_core_service_daemon_client_status_changed_callback(libkrbn_core_service_daemon_client_status_changed_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->register_libkrbn_core_service_daemon_client_status_changed_callback(callback);
    }
  }
}

void libkrbn_unregister_core_service_daemon_client_status_changed_callback(libkrbn_core_service_daemon_client_status_changed_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->unregister_libkrbn_core_service_daemon_client_status_changed_callback(callback);
    }
  }
}

libkrbn_core_service_daemon_client_status libkrbn_core_service_daemon_client_get_status() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      return c->get_status();
    }
  ...
  return libkrbn_core_service_daemon_client_status_none;
}

void libkrbn_core_service_daemon_client_async_temporarily_ignore_all_devices(bool value) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
...
void libkrbn_core_service_daemon_client_async_get_manipulator_environment() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
...
void libkrbn_register_core_service_daemon_client_manipulator_environment_received_callback(libkrbn_core_service_daemon_client_manipulator_environment_received_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->register_libkrbn_core_service_daemon_client_manipulator_environment_received_callback(callback);
    }
  }
}

void libkrbn_unregister_core_service_daemon_client_manipulator_environment_received_callback(libkrbn_core_service_daemon_client_manipulator_environment_received_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->unregister_libkrbn_core_service_daemon_client_manipulator_environment_received_callback(callback);
    }
  }
}

void libkrbn_core_service_daemon_client_async_get_connected_devices() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
...
void libkrbn_register_core_service_daemon_client_connected_devices_received_callback(libkrbn_core_service_daemon_client_connected_devices_received_t _Nonnull callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->register_libkrbn_core_service_daemon_client_connected_devices_received_callback(callback);
    }
  }
}

void libkrbn_unregister_core_service_daemon_client_connected_devices_received_callback(libkrbn_core_service_daemon_client_connected_devices_received_t _Nonnull callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->unregister_libkrbn_core_service_daemon_client_connected_devices_received_callback(callback);
    }
  }
}
...
void libkrbn_register_core_service_daemon_client_notification_message_received_callback(libkrbn_core_service_daemon_client_notification_message_received_t _Nonnull callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->register_libkrbn_core_service_daemon_client_notification_message_received_callback(callback);
    }
  }
}

void libkrbn_unregister_core_service_daemon_client_notification_message_received_callback(libkrbn_core_service_daemon_client_notification_message_received_t _Nonnull callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->unregister_libkrbn_core_service_daemon_client_notification_message_received_callback(callback);
    }
  }
}

void libkrbn_core_service_daemon_client_async_get_system_variables() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
...
void libkrbn_register_core_service_daemon_client_system_variables_received_callback(libkrbn_core_service_daemon_client_system_variables_received_t _Nonnull callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->register_libkrbn_core_service_daemon_client_system_variables_received_callback(callback);
    }
  }
}

void libkrbn_unregister_core_service_daemon_client_system_variables_received_callback(libkrbn_core_service_daemon_client_system_variables_received_t _Nonnull callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
      c->unregister_libkrbn_core_service_daemon_client_system_variables_received_callback(callback);
    }
  }
}

void libkrbn_core_service_daemon_client_async_connect_multitouch_extension() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
...
void libkrbn_core_service_daemon_client_async_clear_user_variables() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_core_service_daemon_client()) {
...
void libkrbn_disable_console_user_server_client() {
  if (auto manager = libkrbn_components_manager_) {
    manager->disable_console_user_server_client();
  }
}

void libkrbn_console_user_server_client_async_start() {
  if (auto manager = libkrbn_components_manager_) {
...
void libkrbn_register_console_user_server_client_status_changed_callback(libkrbn_console_user_server_client_status_changed_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->register_libkrbn_console_user_server_client_status_changed_callback(callback);
    }
  }
}

void libkrbn_unregister_console_user_server_client_status_changed_callback(libkrbn_console_user_server_client_status_changed_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->unregister_libkrbn_console_user_server_client_status_changed_callback(callback);
    }
  }
}

libkrbn_console_user_server_client_status libkrbn_console_user_server_client_get_status() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      return c->get_status();
    }
...
void libkrbn_console_user_server_client_async_get_settings_window_guidance() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
...
void libkrbn_register_console_user_server_client_settings_window_guidance_received_callback(libkrbn_console_user_server_client_settings_window_guidance_received_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->register_libkrbn_console_user_server_client_settings_window_guidance_received_callback(callback);
    }
  }
}

void libkrbn_unregister_console_user_server_client_settings_window_guidance_received_callback(libkrbn_console_user_server_client_settings_window_guidance_received_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->unregister_libkrbn_console_user_server_client_settings_window_guidance_received_callback(callback);
    }
  }
}

void libkrbn_console_user_server_client_async_get_frontmost_application_history() {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->async_get_frontmost_application_history();
    }
  }
}

void libkrbn_register_console_user_server_client_frontmost_application_history_received_callback(libkrbn_console_user_server_client_frontmost_application_history_received_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->register_libkrbn_console_user_server_client_frontmost_application_history_received_callback(callback);
    }
  }
}

void libkrbn_unregister_console_user_server_client_frontmost_application_history_received_callback(libkrbn_console_user_server_client_frontmost_application_history_received_t callback) {
  if (auto manager = libkrbn_components_manager_) {
    if (auto c = manager->get_libkrbn_console_user_server_client()) {
      c->unregister_libkrbn_console_user_server_client_frontmost_application_history_received_callback(callback);
    }
  }
}
